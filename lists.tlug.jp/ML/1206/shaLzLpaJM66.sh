...
#	-f datafile: datafile :-)
#
#	More than one argument can be given. -f is mandatory.
...
do
	...
	d)	DAILY=y;;
	f)	CONFIG="$OPTARG";;
	*)	echo "usage: $0 -f datafile \[-ymwd\]"; exit 1;;
	esac
done

if [ ! -f "$CONFIG" ]
then
	echo "Cannot do anything with no config file (-f option)."
	exit 1
fi
...
echo +++++++++++++++++++++++++++ starting $0 C=${CONFIG} Y=${YEARLY} M=${MONTHLY} W=${WEEKLY} D=${DAILY} at `date`
...
#COPYDIRS="${DOIT} rsync -ar-a -f"+ */" -f"+ .??*/" -f"- *" ./ destination/
...
then
	...
	# copy dirs only, just to be silent...
	#echo "########################## copy dirs only, just to be silent..."
	#rsync \
		#-ai \
		#-f"+ */" \
		#-f"+ .??*/" \
